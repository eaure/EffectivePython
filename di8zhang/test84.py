...
class JpegStreamer(Thread):
    ...
    def send(self, frame):
        ...
        self.lock.acquire()
        if len(self.pipes):
            _, pipees, _ = select([], self.pipes.itervalues(), [], 1)
            print pipees
            for pipe in pipees:
                os.write(pipe, n)
                os.write(pipe, frame)
...
class JpegRetriever(object):
    ...
    def __exit__(self, *args):
        ...
        # self.streamer.unrefgister(self.pipe)
        del self.local.pipe
        # del self.pipe
        return True
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        with self.retriever as frames:
            for frame in frames:
                print "with self.retriever as frames"
...
if __name__ == '__main__':
    ...
    print 'Start server...'
    httpd = ThreadingTCPServer(('127.0.0.1', 9999), Handler)
    # httpd = TCPServer(('127.0.0.1', 9000), Handler)
    httpd.serve_forever()
